...
init {
  atomic {
    run client(client1,loadbalancer1);
    run client(client2,loadbalancer1);
    run client(client3,loadbalancer1);
    run client(client4,loadbalancer2);
    run client(client5,loadbalancer2);
    run client(client6,loadbalancer2);
    
    run server(server1);
    run server(server2);
    run server(server3);

    run loadbalancer(loadbalancer1,server1,server2,server3);
    run loadbalancer(loadbalancer2,server1,server2,server3)
  }
}
