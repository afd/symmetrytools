...
proctype client(chan link) {
  ...
  do
    ...
    :: link!request;
       atomic { link?confirmation; resource_user = _pid; assert(priorities[_pid]<16) };
  ...
  do
    ::
	 ...
	 (link1?[request]||link2?[request]||link3?[request]||
	  ...
	  link10?[request]||link11?[request]||link12?[request]||link13?[request]||
	  link14?[request]||link15?[request]||
	  link16?[request]);
	 ...
	 do
	   ...
	   :: priorities[11]==priority_level && link11?[request] -> client_chan = link11; break
	   :: priorities[12]==priority_level && link12?[request] -> client_chan = link12; break
	   :: priorities[13]==priority_level && link13?[request] -> client_chan = link13; break
	   :: priorities[14]==priority_level && link14?[request] -> client_chan = link14; break
	   :: priorities[15]==priority_level && link15?[request] -> client_chan = link15; break
	   :: priorities[16]==priority_level && link16?[request] -> client_chan = link16; break
