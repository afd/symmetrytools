...
chan database_link = [3] of {mtype,chan};
...
init {
  atomic {
    ...
    run server(servers2,server2_link,database_link);
    ...
    run client(clients6,server2_link);
    run client(clients7,server2_link);
    run client(clients8,server2_link);
  }
}
