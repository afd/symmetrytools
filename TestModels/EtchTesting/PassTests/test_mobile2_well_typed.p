...
mtype = { data, ho_cmd, ho_com, ho_acc, ho_fail, ch_rel, white, red, blue, null };

chan in  = [1] of { mtype };
chan out = [1] of { mtype };
chan fa  = [0] of { mtype, chan };
chan fp  = [0] of { mtype, chan };
chan m1  = [0] of { mtype, chan };
chan m2  = [0] of { mtype, chan };
chan l   = [0] of { chan };
chan null_chan = [0] of { mtype, chan };
...
{	chan  m_old, m_new, x;
	...
	:: in?v ->
		printf("MSC: DATA\n");
		fa!data,null_chan; fa!v,null_chan
	:: l?m_new ->
		fa!ho_cmd,null_chan;
                fa!null,m_new;
		printf("MSC: HAND-OFF\n");
		if
		:: fp?ho_com,eval(null_chan) ->
			printf("MSC: CH_REL\n");
			fa!ch_rel,null_chan; fa?eval(null),m_old;
			l!m_old;
			x = fa; fa = fp; fp = x
		:: fa?ho_fail,eval(null_chan) ->
			printf("MSC: FAIL\n");
			l!m_new
		fi
	od
...
Active:
	printf("MSC: ACTIVE\n");
	do
	:: f?data,eval(null_chan) -> f?eval(null),v; m!data,null_chan; m!null,v
	:: f?ho_cmd,eval(null_chan) ->	/* handover command */
progress:	f?eval(null),v; m!ho_cmd,null_chan; m!null,v;
		if
		:: f?ch_rel,eval(null_chan) ->
			f!null,m;
			goto Passive
		:: m?ho_fail,eval(null_chan) ->
			printf("MSC: FAILURE\n");
			f!ho_fail,null_chan
		fi
	od;
...
Passive:
	printf("MSC: PASSIVE\n");
	m?ho_acc,eval(null_chan) -> f!ho_com,null_chan;
	goto Active
}
...
{	chan m_new;
	...
	do
	:: m?data,eval(null_chan) -> m?v,eval(null_chan); out!v
	:: m?ho_cmd,eval(null_chan); m?eval(null),m_new;
		if
		:: m_new!ho_acc,null_chan; m = m_new
		:: m!ho_fail,null_chan
		fi
	od
}
